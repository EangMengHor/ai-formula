...
export default function RatioSlider({
  ...
  const handleSliderChange = (index, values) => {
    ...
    if (isComingFromMax && newValue < 100) {
      ...
      const otherIndices = Array.from({ length: newPercentages.length }, (_, i) => i).filter((i) => i !== index)
      ...
      const otherTotal = otherIndices.reduce((sum, i) => sum + percentages[i], 0)
  ...
  const handleRemove = (index) => {
    ...
    if (totalRemaining > 0) {
      ...
      for (let i = 0; i < newPercentages.length; i++) {
        newPercentages[i] = newPercentages[i] + (newPercentages[i] / totalRemaining) * removedPercentage
      }
    } else {
  ...
  const getFilteredOptions = () => {
    ...
    return selectableOptions.filter((option) => {
      const optionValue = typeof option === "object" ? option[displayKey] : String(option)
    ...
    <div className={`space-y-3 ${className}`}>
      <AnimatePresence>
        {items.map((item, index) => (
          <motion.div
            key={typeof item === "object" && item !== null ? item.id || index : `${item}-${index}`}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-purple-400 font-medium">{percentages[index]?.toFixed(1)}%</span>
              </div>
      ...
      {allowAdd && (
        <div className="space-y-2 mt-4">
              ...
              <div className="flex-1">
                <Popover open={dropdownOpen} onOpenChange={setDropdownOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <span className="truncate">{searchValue || newItemPlaceholder}</span>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {getFilteredOptions().map((option, index) => {
                            const optionDisplay = typeof option === "object" ? option[displayKey] : String(option)
                            const optionValue = typeof option === "object" ? option[valueKey] : option
                            const isSelected = isOptionSelected(option)

                            return (
                              <CommandItem
                                key={typeof optionValue === "object" ? index : optionValue}
                                onSelect={() => !isSelected && handleSelectItem(option)}
                                className={cn(
          ...
          {newItemError && <p className="text-red-400 text-sm">{newItemError}</p>}
          {!selectableOptions && getDataTypeHelper() && <p className="text-xs text-slate-500">{getDataTypeHelper()}</p>}
        </div>
      )}
    </div>
  )
