...
function extractSectionAndInstruction(blockText) {
  ...
  const instructionMatch = blockText.match(
    /^- instruction:\s*([\s\S]*?)(?=\n- [a-z])/m,
  );
      ...
      try {
        ...
        if (res?.success) {
          ...
          if (
            ...
            setIsCompleted(
              res.data.content[res.data.content.length - 1]?.isCompleted ||
                false,
            );
          }
      ...
      <div className="p-3">
        <div className="p-3 bg-red-500/20 border border-red-500 text-red-200 rounded-2xl">
          <p className="font-medium">
            Error: Failed to fetch document generation status
          </p>
          <button
            onClick={() => {
              setIsError(false);
    ...
    <div className="p-3 text-white">
      ...
      >
        <TabsList className="bg-slate-700 rounded-xl flex gap-2 w-fit sticky top-0 z-50">
          ...
          <TabsTrigger value="task" className="text-white">
            <div className="flex gap-2 items-center">
        ...
        <TabsContent value="pdf" className="overflow-hidden">
          {/* export section */}
          <div
            className={`${workflowData?.fileBucketPath ? "w-full" : "hidden"}  p-2 border-white/40 border-2 rounded-xl mb-2`}
          >
            {/* label */}
            ...
            <div className="flex gap-2 mt-2">
              ...
              <div className="bg-slate-600 hover:bg-slate-400 px-2 py-1 rounded-lg">
                <a
                  href={`https://docs.google.com/viewerng/viewer?url=${workflowData?.fileBucketPath}`}
            ...
            {workflowData?.fileBucketPath ? (
              <>
                ...
                <div className="hidden p-4 text-center text-gray-500 bg-gray-100 rounded-lg">
                  <p>
                    Unable to display PDF. Please try opening it in a new tab.
                  </p>
                  <a
        ...
        <TabsContent value="task">
          <div className="bg-g1/70 p-3 rounded-2xl flex-col flex justify-between gap-2 relative">
            <div className="flex gap-2 items-center justify-between">
              <div className="flex gap-2 items-center">
                <CircleDashed
                  className={`${!isCompleted && "animate-spin"} w-5 h-5`}
                />
                <p>Summary</p>
              </div>
              {!isCompleted && (
                <div className="text-yellow-500 text-sm">
                  {workflowData?.progress?.estimatedRemainingTime || "0"}{" "}
                  Remaining
                </div>
              )}
            </div>
            ...
            <StandardWorkflow
              data={
                workflowData?.content?.map((item) => ({
                  title:
                    extractSectionAndInstruction(item.sectionInfo).section ||
                    "No Title",
                  description:
                    extractSectionAndInstruction(item.sectionInfo)
                  ...
                  children: (
                    <div className="flex gap-2">
                      ...
                      {item.internetSearch &&
                        item.internetSearch.length > 0 && (
                          <div className="flex items-center gap-2 bg-slate-900 rounded-xl p-2 w-fit">
                            <Globe2 className="w-6 h-6" />
                            <p>
                              {item.internetSearch.length} Internet Searches
                            </p>
                          </div>
                        )}
                    </div>
