...
export default function TaxStrategiesStep({ formData, setFormData }) {
    ...
    const handleTaxStrategyChange = (id) => {
        ...
        if (currentStrategies.includes(id)) {
            setFormData({
                ...formData,
                taxOptimizationStrategies: currentStrategies.filter((item) => item !== id),
            })
        } else {
            setFormData({
                ...formData,
    ...
    return [
        <div className="flex">
            <div key="form" className="space-y-6">
                <div>
                    <h1 className="text-3xl font-bold mb-2">Tax Optimization Strategies</h1>
                    <p className="text-gray-400">Select the tax strategies you're currently using or interested in.</p>
                </div>
                        ...
                        {taxStrategies.map((strategy) => (
                            <div key={strategy.id} className="flex items-center space-x-2">
                                <Checkbox
                                    ...
                                    checked={formData.taxOptimizationStrategies.includes(strategy.id)}
                                    onCheckedChange={() => handleTaxStrategyChange(strategy.id)}
                                    className="border-gray-600 data-[state=checked]:bg-blue-600"
                                />
                                <label htmlFor={`strategy-${strategy.id}`} className="text-sm font-medium leading-none text-gray-300">
                                    {strategy.label}
                                </label>
                            </div>
                        ))}
                ...
                <div className="mt-8">
                    <p className="text-blue-400 text-sm">
                        "AI will incorporate your tax strategies to optimize after-tax returns and suggest additional tax-efficient
                        approaches based on your profile."
                    </p>
                </div>
            </div>
                ...
                <div className="w-md h-full max-h-full bg-[#2a3042] rounded-lg flex items-center justify-center p-6">
                    {formData.taxOptimizationStrategies.length > 0 ? (
                        <div className="w-full h-full">
                            <div className="text-lg font-bold text-white mb-4 text-center">Tax Optimization</div>
                            <div className="space-y-3 max-h-40 overflow-y-auto">
                                {formData.taxOptimizationStrategies.map((strategyId) => {
                                    const strategy = taxStrategies.find((s) => s.id === strategyId)
                                    return (
                                        <div key={strategyId} className="bg-blue-600/20 rounded-lg p-2 flex items-center">
                                            <Calculator className="h-4 w-4 text-blue-400 mr-2" />
                                            <span className="text-sm text-blue-300">{strategy?.label}</span>
                                        </div>
                                    )
                                })}
                            </div>
