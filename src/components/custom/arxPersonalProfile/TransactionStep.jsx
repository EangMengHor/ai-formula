...
export default function TransactionStep({ formData, setFormData }) {
  ...
  const removeTransaction = (id) => {
    setFormData({
      ...formData,
      transactions: formData.transactions.filter(
    ...
    <div className="w-full flex">
      <div key="form" className="space-y-6">
        <div>
          ...
          <p className="text-gray-400">
            Add any recent transactions to help us understand your investment
            behavior (optional).
          </p>
        </div>
        ...
        <div className="mt-8">
          <p className="text-blue-400 text-sm">
            "AI will analyze your transaction patterns to understand your
            investment behavior and identify potential biases or opportunities."
          </p>
        </div>
      </div>
      ...
      >
          ...
          {formData.transactions.length > 0 ? (
            <div className="w-full h-full">
              ...
              <div className="flex justify-between items-end h-32 mb-4">
                {["buy", "sell"].map((type) => {
                  const count = formData.transactions.filter(
                    (t) => t.type === type,
                  ).length;
                  const maxCount = Math.max(
                    formData.transactions.filter((t) => t.type === "buy")
                      .length,
                    formData.transactions.filter((t) => t.type === "sell")
                      .length,
                    1,
                  );
        ...
        <p className="mt-6 text-gray-400 text-center max-w-xs">
          AI analyzes your transaction patterns to understand your investment
          behavior
        </p>
      </div>
    </div>,
