...
export default function SecurityStep({ formData, setFormData }) {
  ...
  const addSecurity = () => {
    if (newSecurity.ticker && newSecurity.quantity) {
      setFormData({
        ...formData,
        securities: [...formData.securities, { ...newSecurity, id: Date.now() }],
      })
      setNewSecurity({
        ticker: "",
        quantity: "",
      ...
      <div className="mt-8 space-y-4">
        ...
        <div className="mt-4 space-y-2">
          {formData.securities.map((security) => (
            <div key={security.id} className="flex justify-between items-center bg-[#2a3042] p-3 rounded-md">
              <div>
                <span className="font-medium text-white">{security.ticker}</span>
                <span className="ml-2 text-gray-400">{security.quantity} shares</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
      ...
      <div className="w-64 h-64 bg-[#2a3042] rounded-lg flex items-center justify-center p-6">
        {formData.securities.length > 0 ? (
          <div className="w-full h-full">
            <div className="text-lg font-bold text-white mb-4 text-center">Your Portfolio</div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-700">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-medium">{formData.securities.length}</span>
              </div>
            </div>
          </div>
        ) : (
