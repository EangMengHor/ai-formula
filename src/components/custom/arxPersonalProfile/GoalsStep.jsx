...
export default function GoalsStep({ formData, setFormData }) {
    ...
    const goalOptions = [
        { id: "alpha", label: "Alpha Generation", description: "Outperform the market" },
        { id: "taxEfficiency", label: "Tax Efficiency", description: "Minimize tax burden" },
        { id: "esgCompliance", label: "ESG Compliance", description: "Sustainable investing" },
        { id: "volatilityReduction", label: "Volatility Reduction", description: "Smoother returns" },
        { id: "incomeGeneration", label: "Income Generation", description: "Regular cash flow" },
        { id: "capitalPreservation", label: "Capital Preservation", description: "Protect principal" },
    ]
    ...
    const addCustomGoal = () => {
        if (newGoal.trim()) {
            setFormData({
                ...formData,
                optimizationGoals: [...formData.optimizationGoals, `custom-${newGoal.trim()}`],
            })
            setNewGoal("")
        }
    }
    ...
    return (
        <div className="flex">
            <div key="form" className="space-y-6">
                <div>
                    <h1 className="text-3xl font-bold mb-2">What are your investment goals?</h1>
                    <p className="text-gray-400">Select the objectives you want to prioritize in your investment strategy.</p>
                </div>
                ...
                <div className="mt-8 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                        {goalOptions.map((goal) => (
                            <Button
                                key={goal.id}
                                type="button"
                                variant={formData.optimizationGoals.includes(goal.id) ? "default" : "outline"}
                                className={`justify-start  text-left py-8 ${formData.optimizationGoals.includes(goal.id)
                            ...
                            >
                                <div>
                                    <div className="flex items-center">
                                        {formData.optimizationGoals.includes(goal.id) && <Check className="h-4 w-4 mr-2" />}
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {formData.optimizationGoals
                            ...
                            .map((goal) => (
                                <div key={goal} className="flex items-center bg-blue-600 text-white px-3 py-1 rounded-full text-sm">
                                    {goal.replace("custom-", "")}
                                    <Button
                                        variant="ghost"
                                        size="sm"
                ...
                <div className="mt-8">
                    <p className="text-blue-400 text-sm">
                        "AI will prioritize your selected goals when building and optimizing your investment strategy, balancing
                ...
                <div className="w-64 h-64 bg-[#2a3042] rounded-lg flex items-center justify-center p-6">
                    {formData.optimizationGoals.length > 0 ? (
                        <div className="w-full h-full">
                            <div className="text-lg font-bold text-white mb-4 text-center">Investment Goals</div>
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-700">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-white font-medium">{formData.optimizationGoals.length}</span>
                                </div>
                            </div>
                        </div>
                    ) : (
