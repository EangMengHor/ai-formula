...
export default function AllocationStep({ formData, setFormData }) {
  ...
  const addAllocation = () => {
    if (newAllocation.category && (newAllocation.strategic || newAllocation.actual)) {
      ...
      if (newAllocation.strategic) {
        updatedStrategic[newAllocation.category] = Number.parseFloat(newAllocation.strategic)
      }

      if (newAllocation.actual) {
        updatedActual[newAllocation.category] = Number.parseFloat(newAllocation.actual)
      }
  ...
  const addCommitment = () => {
    if (newCommitment.fund && newCommitment.commitmentAmount) {
      setFormData({
        ...formData,
        privateEquityCommitments: [...formData.privateEquityCommitments, { ...newCommitment, id: Date.now() }],
      })
      setNewCommitment({
        fund: "",
        commitmentAmount: "",
  ...
  const removeCommitment = (id) => {
    setFormData({
      ...formData,
      privateEquityCommitments: formData.privateEquityCommitments.filter((commitment) => commitment.id !== id),
    })
  }
  ...
  return (
    <div className="flex">
      ...
      <div>
        ...
        <p className="text-gray-400">Enter your strategic (target) and actual asset allocations (optional).</p>
      </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="flex space-x-2">
            ...
            <Button
              ...
              disabled={!newAllocation.category || (!newAllocation.strategic && !newAllocation.actual)}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-700/30">
                  <tr>
                    ...
                    <th className="text-right py-2 px-3 text-gray-300">Strategic %</th>
                    <th className="text-right py-2 px-3 text-gray-300">Actual %</th>
      ...
      <div className="mt-8 space-y-4">
        <Label className="text-gray-300 text-lg">Private Equity / VC Commitments</Label>
          ...
          {formData.privateEquityCommitments.length > 0 ? (
            <div className="space-y-2">
              {formData.privateEquityCommitments.map((commitment) => (
                <div key={commitment.id} className="flex justify-between items-center bg-[#2a3042] p-3 rounded-md">
                  <div>
                    <span className="font-medium text-white">{commitment.fund}</span>
                    <span className="ml-2 text-gray-400">
                      ${commitment.commitmentAmount} ({commitment.vintage || "N/A"})
                    </span>
                  </div>
                  <Button
                    variant="ghost"
      ...
      <div className="mt-8">
        <p className="text-blue-400 text-sm">
          "AI will analyze the gap between your strategic and actual allocations to identify rebalancing opportunities
          and optimize your portfolio structure."
        </p>
      </div>
    </div>
      ...
      <div className="w-64 h-64 bg-[#2a3042] rounded-lg flex items-center justify-center p-6">
        {getAllocationCategories().length > 0 ? (
          <div className="w-full h-full">
            <div className="text-lg font-bold text-white mb-4 text-center">Asset Allocation</div>

            <div className="relative h-40">
              <div className="absolute inset-0">
                <svg viewBox="0 0 100 100" className="w-full h-full">
                  {getAllocationCategories().map((category, index, array) => {
                    const strategicValue = formData.strategicAllocation[category] || 0
                    const actualValue = formData.actualAllocation[category] || 0

                    // Calculate positions for pie segments
                    const strategicTotal =
                      Object.values(formData.strategicAllocation).reduce((sum, val) => sum + val, 0) || 100
                    const actualTotal =
                      Object.values(formData.actualAllocation).reduce((sum, val) => sum + val, 0) || 100

                    const strategicStartAngle = array
                      .slice(0, index)
                      .reduce((sum, cat) => sum + ((formData.strategicAllocation[cat] || 0) / strategicTotal) * 360, 0)
                    const strategicEndAngle = strategicStartAngle + (strategicValue / strategicTotal) * 360

                    const actualStartAngle = array
                      .slice(0, index)
                      .reduce((sum, cat) => sum + ((formData.actualAllocation[cat] || 0) / actualTotal) * 360, 0)
                    const actualEndAngle = actualStartAngle + (actualValue / actualTotal) * 360
                    ...
                    if (strategicValue > 0 || actualValue > 0) {
                      return (
                        <g key={category}>
                          ...
                          {strategicValue > 0 && (
                            <path
                              d={describeArc(50, 50, 40, strategicStartAngle, strategicEndAngle)}
                              fill={strategicColor}
                              stroke="#242938"
                              strokeWidth="1"
                            />
                          ...
                          {actualValue > 0 && (
                            <path
                              d={describeArc(50, 50, 30, actualStartAngle, actualEndAngle)}
                              fill={actualColor}
                              stroke="#242938"
                              strokeWidth="1"
                            />
          ...
          <div className="text-gray-500 text-center">
            ...
            <div className="text-sm mt-2">to see your portfolio breakdown</div>
          </div>
        )}
      </div>
