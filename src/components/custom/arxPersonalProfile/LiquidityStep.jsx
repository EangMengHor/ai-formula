...
export default function LiquidityStep({ formData, setFormData }) {
  const liquidityOptions = [
    ...
    { value: "long-term", label: "Long Term", description: "3+ years" },
    {
      ...
      description: "Regular access to funds",
    },
    {
      value: "quarterly",
      label: "Quarterly Liquidity",
      description: "Periodic access to funds",
    },
  ];
  ...
  return (
    <div className="flex">
      <div key="form" className="space-y-6">
        <div>
          ...
          <p className="text-gray-400">
            Your investment time horizon helps determine appropriate strategies.
          </p>
        </div>
        ...
        <div className="mt-8">
          <p className="text-blue-400 text-sm">
            "AI will factor in your liquidity needs to ensure your investments
            align with your time horizon, balancing growth with accessibility."
          </p>
        </div>
      </div>
      ...
      <div
        key="visual"
        className="flex w-1/2 flex-col items-center justify-center h-full "
      >
          ...
          {formData.liquidityHorizon ? (
            <div className="w-full h-full flex flex-col items-center justify-center">
              ...
              <div className="text-center">
                ...
                {(formData.liquidityHorizon === "monthly" ||
                  formData.liquidityHorizon === "quarterly") && (
                  <div className="flex justify-between w-full mt-4">
                    {Array.from({
                      length: formData.liquidityHorizon === "monthly" ? 12 : 4,
                    }).map((_, i) => (
                      <div
                        key={i}
                ...
                <div className="mt-4 text-sm text-gray-400">
                  ...
                  {formData.liquidityHorizon === "medium-term" &&
                    "Access in 1-3 years"}
                  {formData.liquidityHorizon === "long-term" &&
                    "Access after 3+ years"}
                  {formData.liquidityHorizon === "monthly" &&
                    "Monthly access to funds"}
                  {formData.liquidityHorizon === "quarterly" &&
                    "Quarterly access to funds"}
                </div>
              </div>
            </div>
          ) : (
        ...
        <p className="mt-6 text-gray-400 text-center max-w-xs">
          Your liquidity needs help AI balance growth potential with access to
          capital
        </p>
      </div>
    </div>
