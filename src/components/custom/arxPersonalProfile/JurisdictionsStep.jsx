...
export default function JurisdictionsStep({ formData, setFormData }) {
  ...
  const addJurisdiction = () => {
    if (
      newJurisdiction.trim() &&
      !formData.jurisdictions.includes(newJurisdiction.trim())
    ) {
      setFormData({
        ...formData,
    ...
    <div className="flex ">
      <div key="form" className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">
            What jurisdictions are relevant to you?
          </h1>
          <p className="text-gray-400">
            Add any additional jurisdictions where you have tax obligations or
        ...
        <div className="mt-8 space-y-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {formData.jurisdictions.map((jurisdiction, index) => (
              <div
                key={index}
                className="flex items-center bg-[#2a3042] text-white px-3 py-1 rounded-full text-sm"
              >
                {jurisdiction}
                <Button
                  variant="ghost"
        ...
        <div className="mt-8">
          <p className="text-blue-400 text-sm">
            "AI will consider multi-jurisdictional tax implications and
            regulatory requirements when optimizing your investment strategy."
          </p>
        </div>
      </div>
      ...
      >
            ...
            <div className="w-full h-full">
              ...
              <div className="relative w-full h-32">
                ...
                {formData.jurisdictions.map((jurisdiction, index) => {
                  ...
                  const angle =
                    index *
                    (360 / formData.jurisdictions.length) *
                    (Math.PI / 180);
                  const radius = 40;
        ...
        <p className="mt-6 text-gray-400 text-center max-w-xs">
          Multi-jurisdictional considerations help AI optimize tax efficiency
          across borders
        </p>
      </div>
    </div>
