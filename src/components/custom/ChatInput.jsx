...
function ChatInput({
    ...
    const { selectedWorkflowId, workflowList, setWorkflowList, getSelectedWorkflow, setSelectedWorkflowId } = useWorkflow();

    // Move remaining workflow states here to prevent unnecessary rerenders
    const [workflowPrompt, setWorkflowPrompt] = useState("");
    const [isWorkflowCreatorLoading, setIsWorkflowCreatorLoading] = useState(false);
    const [recentlyCreatedWorkflowResponse, setRecentlyCreatedWorkflowResponse] = useState({});
    ...
    const handleCreateNewWorkflow = useCallback(async () => {
        try {
            ...
            if (!user || !user.id || !conversationProp.current || conversationProp.current.length === 0) {
                console.log(!user || !user.id || !conversation || conversation.length === 0, "User or conversation is not available");
                toast({
                    title: "Error",
                    description: "Please ensure you have a conversation,  and personas selected before creating a new workflow.",
                    variant: "destructive",
                });
                return;
            }
            ...
            let nextId = 1;
            const agents = conversation.reduce((acc, msg) => {
                if (msg.role !== "ai") return acc;

                const simulationBlocks = msg.message.filter(b => b.type === "simulation");
    ...
    const getVoiceAgentUrl = useCallback((baseUrl) => {
        if (!baseUrl) return '';

        if (files.length === 0) {
            return id && id !== undefined
                ? `${baseUrl}?namespace=${id}`
                : baseUrl;
        }
    ...
    const handleWorkflowModalChange = useCallback((value) => {
                            ...
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    ...
                                    >
                                        ...
                                        <div className="z-10">
                                            <CircleFadingPlus
                                                className={`w-5 h-5 ${selectedWorkflowId
                                                    ? "text-white "
                                                    : "text-white"
                                ...
                                <Drawer>
                                    ...
                                    <DrawerContent className="bg-slate-600 flex flex-col gap-2">
                                        <DrawerHeader>
                                            ...
                                            <DrawerDescription>Choose a voice agent for your conversation</DrawerDescription>
                                        </DrawerHeader>
                                        <div className="px-4 flex flex-col gap-2">
                                            <div
                                                onClick={() => {
                                                    const url = getVoiceAgentUrl(import.meta.env.VITE_OPENAI_REALTIME_URL);
                            ...
                            <Dialog>
                                ...
                                <DialogContent className="max-w-5xl bg-slate-700">
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>
                                            Choose a voice agent to enhance your conversation experience
                                        </DialogDescription>
                                    </DialogHeader>
            ...
            >
                <DialogContent className="bg-[#1e2535] text-white border border-slate-700  w-[40%] h-fit max-h-[60%] overflow-y-scroll">
                    ...
                    <div>
                        ...
                        <p className="font-thin text-slate-400">Turn this chat into a workflow. AI will review the messages and build a new one for you.</p>
                    </div>
                    <hr />
                    ...
                    {
                        recentlyCreatedWorkflowResponse && Object.keys(recentlyCreatedWorkflowResponse).length > 0 && (
                            <div>
                                <div className="bg-slate-800 text-white p-6 max-w-3xl mx-auto rounded">
                                    <h2 className="text-xl font-medium mb-1">{recentlyCreatedWorkflowResponse.name}</h2>
                                    <p className="text-slate-300 text-sm mb-4">{recentlyCreatedWorkflowResponse.description}</p>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="space-y-6">
                                            {recentlyCreatedWorkflowResponse.workflow.map((step, index) => (
                                                <div
                                                    key={index}
                                                ...
                                                >
                                                    ...
                                                    {/* Simple connector */}
                                                    {index < recentlyCreatedWorkflowResponse.workflow.length - 1 && (
                                    ...
                                    <div className="mt-6 space-y-4">
                                        {
                                            recentlyCreatedWorkflowResponse &&
                                                Object.keys(recentlyCreatedWorkflowResponse).includes("personaList") &&
                                                recentlyCreatedWorkflowResponse.personaList.length > 0 ? (
                                                <div>
                                                    <p className="py-2 font-semibold mb-2">Agents</p>
                                                    <ul className="space-y-2">
                                                        {recentlyCreatedWorkflowResponse.personaList.map((agent, index) => (
