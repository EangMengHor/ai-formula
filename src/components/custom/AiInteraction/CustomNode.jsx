...
export function CustomNode({ data }) {
  ...
  return (
    <div
      className={`py-4 px-4 shadow-lg rounded-lg ${getBgColor()} border-2 ${getBorderColor()} w-[300px] `}
    >
      <Handle
        type="target"
      ...
      {!data.isStart && !data.isEnd && (
        <div className="mt-2 text-sm text-slate-300 ">{data.goal}</div>
      )}
      {data?.isAgenticWorkflowExecuted && (
        <div>
          ...
          <p className="text-xs my-2 text-slate-400">
            Agent Have Executed Workflow Given By User To Archive This Goal!
          </p>
        </div>
      )}
      <Handle
