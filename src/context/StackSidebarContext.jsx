...
export default function StackSidebarProvider({ children }) {
  ...
  function setSidebarStack(val) {
    ...
    if (isAnimatingRef.current) {
      ...
      setsidebarStackData(typeof val === "function" ? val(sidebarStack) : val);
      return;
    }

    const newStack = typeof val === "function" ? val(sidebarStack) : val;
    ...
    const isPop =
      ...
      sidebarStack
        .slice(0, newStack.length)
        .every((item, i) => item === newStack[i]);
...
export const useStackSidebar = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useStackSidebar must be used within a StackSidebarProvider",
    );
  }
  return context;
};
