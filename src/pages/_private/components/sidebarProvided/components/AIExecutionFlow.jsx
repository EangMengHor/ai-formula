  ...
  const handleSocketEvent = (event) => {
    ...
    if (event.type === "defineGoal") {
      ...
      setSteps((prevSteps) => {
        ...
        lastStep.goal =
          lastStep.goal && lastStep.goal !== undefined
            ? lastStep.goal + event.content || ""
            : event.content || ""; // Update the goal with the content from the event
      ...
      {!steps.length && (
        <div className="p-4">
          <button
            onClick={() =>
              sendMessage(
                "Provide a very detailed plan to acquire Lamborghini Company including the purchase price and all the details that can be funneled",
                "b4375d98-4794-479d-8a42-8d8d5da38e51",
                "large",
                false,
                ["226472ff-5a25-4449-be1c-ac84051657a7"],
                false,
              )
            }
