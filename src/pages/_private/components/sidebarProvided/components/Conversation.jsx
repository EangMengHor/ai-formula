import React, { memo, useEffect, useImperativeHandle, forwardRef, useState } from "react";
...
import { RotateCcw, FileDown, Loader2, Copy, Check, Download } from "lucide-react";
...
const Conversation = forwardRef(({
    ...
    useEffect(() => {
        if (bottomRef.current && conversation[conversation.length - 1]?.role !== "ai") {
    ...
    const copyToClipboard = async (content) => {
        try {
            ...
            const plainText = content.replace(/\*\*(.*?)\*\*/g, '$1')  // Bold
                .replace(/\*(.*?)\*/g, '$1')       // Italic
                .replace(/\[(.*?)\]\((.*?)\)/g, '$1: $2') // Links
                .replace(/#{1,6}\s(.*?)(\n|$)/g, '$1\n') // Headers
                .replace(/```[a-zA-Z]*\n([\s\S]*?)```/g, '$1') // Code blocks
...
${block.content}
    ...
    const renderTextBlock = (block, blockIdx, isLastBlock, currContent, isDeepThink = false) => {
    ...
    const renderDocumentBlock = (block, blockIdx) => (
        <div
            onClick={() => handleBlockSidebar(block.content, block.type, block.name)}
            key={`doc-${blockIdx}`}
    ...
    const renderVisualBlock = (block, blockIdx) => {
        ...
        return (
            <div
                onClick={() => handleBlockSidebar(sanitizedMermaid, block.type, block.name)}
                key={`visual-${blockIdx}`}
                className={`border-2 border-slate-800 bg-slate-900 flex justify-between items-center gap-2 relative rounded-lg p-1 ${block.isComplete ? "cursor-pointer hover:bg-slate-800 text-white flex" : ""
                    }`}
            >
                    ...
                    return (
                        ...
                        >
                            ...
                            {Array.isArray(item.message) && item.message.map((block, blockIdx) => {
                                const isLastBlock = blockIdx === item.message.length - 1 && (!isNextChatLoading || (conversation.length !== index + 1));
                                ...
                                if (block.type === "simulation") {
                                    ...
                                    return renderTextBlock(block, blockIdx, isLastBlock, currContent, true);
                    ...
                    return (
                        ...
                        >
                            ...
                            {Array.isArray(item.message) && item.message.map((block, blockIdx) => {
                                ...
                                if (block.type === "text") {
                                    return renderTextBlock(block, blockIdx, isLastBlock, currContent);
                                } else if (block.type === "mermaid") {
                                    return renderMermaidBlock(block, blockIdx);
