...
export function StreamingResponse({ content }) {
    ...
    useEffect(() => {
        ...
        if (processedBlocks.length !== blocks.length || JSON.stringify(processedBlocks) !== JSON.stringify(blocks)) {
    ...
    const openDialog = (block) => {
        ...
        setDialogTitle(block.name || (block.type === "visual" ? "Visualization" : "Document"))
        setDialogOpen(true)
    }
            ...
            {blocks.map((block) => (
                <div key={block.id}>
                    ...
                    {block.type === "visual" && (
                        ...
                        >
                            <div className="text-sm font-medium text-gray-700 mb-2">{block.name || "Visualization"}</div>

                            {!block.isComplete ? (
                                <div className="flex items-center justify-center p-8">
                                    <Loader2 className="h-8 w-8 animate-spin text-blue-500" />
                                    <span className="ml-2 text-sm text-gray-500">Loading visualization...</span>
                                </div>
                            ) : (
                                <>
                                    <div className="text-xs text-gray-500 mb-2">(click to expand)</div>
                    ...
                    {block.type === "document" && (
                        <div
                            ...
                            onClick={() => block.isComplete && openDialog(block)}
                        >
                            <div className="text-sm font-medium text-gray-700 mb-2">{block.name || "Document"}</div>
