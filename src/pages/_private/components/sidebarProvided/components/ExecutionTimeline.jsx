...
import { Clock, Search, Database, CheckCircle, ChevronDown, ExternalLink, Plus, X } from "lucide-react"
            ...
            {url && typeof url == 'object' && Object.keys(url) > 0 ? (
                ...
                <div className="space-y-1">
                    <div className="flex items-center">
                        ...
                        <div className="text-sm font-bold text-white truncate">{url?.title}</div>
                    </div>
                    <div className="text-xs text-slate-400 line-clamp-2">{url?.description}</div>
                    <a
                        href={`${url.url}`}
                        target="_blank"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            ...
            >
                <div className="flex justify-between items-center mb-4 sticky top-0 bg-[#0a0c14] z-10 py-2">
                    <h3 className="text-lg font-semibold text-white">All Internet Search Results</h3>
...
const TypeWriter = ({ text, isCoT = false }) => {
    ...
    return (
        <div className="mb-2 ">
            {isCoT ? displayText.split('.').map((line, index) => (
                <div key={index} className="mb-2">
                    <ReactMarkdown
                        ...
                        components={{
                            p: ({ children }) => <p className="text-purple-50">{children}</p>,
                            table: ({ children }) => (
                                <table style={{ borderCollapse: "collapse", width: "100%", color: "#e0e0e0" }}>
                                    {children}
                                </table>
            ...
            {steps.map((step, idx) => (
                ...
                >
                    <div className="text-slate-300 italic">
                        {isLatest && idx === steps.length - 1 ? <TypeWriter text={step.trim()} isCoT={true} /> : step.trim().split('.').map((line, index) => (
    ...
    return (
        ...
        >
            ...
            <div className="text-xs text-slate-400">
                <TruncatedText text={isLatest ? <TypeWriter text={item.text} /> : item.text} maxLength={120} />
            </div>
        </motion.div>
    )
}
...
const InternetSearchResults = ({ urls, isLatest, isLoading }) => {
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {dialogOpen && <SearchResultsDialog isOpen={dialogOpen} onClose={() => setDialogOpen(false)} urls={urls} />}
            </AnimatePresence>
        </>
    )
}
...
const Step = ({ step, isLast, isLatest, isNew }) => {
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className={`h-6 w-6 rounded-full flex items-center justify-center ${step.type === "finish" ? "bg-purple-700" : "bg-slate-800"
                        }`}
                >
                    {step.type === "finish" && <CheckCircle className="h-3 w-3 text-white" />}
                </div>
            </motion.div>
            ...
            <div>
                {step.type === "defineGoal" && (
                    <motion.div
                        ...
                        className="mb-4"
                    >
                        <h3 className="text-lg font-semibold mb-2 text-white">Defining End Goal</h3>
                        <div className="text-slate-300 mb-2 bolder">
                            <TruncatedText text={isLatest ? <TypeWriter text={step.text || ""} /> : step.text || ""} maxLength={80} />
                        </div>
                ...
                {step.type === "thinking" && (
                    <motion.div
                        ...
                        className="mb-4"
                    >
                        <h3 className="text-lg font-semibold mb-2 text-white">Thinking...</h3>
                    ...
                    >
                        ...
                        {step.goal && (
                            ...
                            >
                                ...
                                <div className="text-slate-300">
                                    <TruncatedText text={isLatest ? <TypeWriter text={step.goal} isCoT={true} /> : step.goal} maxLength={100} />
                                </div>
                            </motion.div>
                        )}
                                ...
                                <div className="space-y-2">
                                    {step.isLoadingKnowledge ? (
                                        ...
                                        step.knowledgeBase.map((item, kbIndex) => (
                                            <KnowledgeBaseItem key={kbIndex} item={item} index={kbIndex} isLatest={isLatest} />
                                        ))
                                    )}
                                </div>
                            </motion.div>
                        ...
                        {(step.search || step.isLoadingSearch) && (
                            ...
                            >
                                ...
                                <InternetSearchResults urls={step.search} isLatest={isLatest} isLoading={step.isLoadingSearch} />
                            </motion.div>
                        )}
                    </motion.div>
                )}
                ...
                {step.type === "reEvaluating" && (
                    <motion.div
                        ...
                        className="mb-4"
                    >
                        <h3 className="text-lg font-semibold mb-2 text-white">Re-evaluating</h3>
                        <div className="text-slate-300 italic">
                            {isLatest ? <TypeWriter text={step.text || ""} /> : step.text || ""}
                        </div>
                    </motion.div>
                )}
                ...
                {step.type === "finish" && (
                    <motion.div
                        ...
                        className="mb-4"
                    >
                        <h3 className="text-lg font-semibold text-purple-400">Writing Response...</h3>
                    </motion.div>
                )}
            </div>
        </motion.div>
            ...
            <div className="w-full bg-[#0a0c14] rounded-lg overflow-hidden">
                ...
                <AnimatePresence>
                    {!collapsed && (
                        ...
                        >
                            <div ref={timelineRef} className="relative p-6 max-h-[80vh] overflow-y-auto">
