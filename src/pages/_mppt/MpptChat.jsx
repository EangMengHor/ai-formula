    ...
    return (
        <div className="bg-white/5 rounded-xl p-3 space-y-1.5">
            <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-xs text-gray-500 flex-shrink-0">P{task.phase}</span>
            </div>
            {task.goal && (
                <>
...
function BranchesViewer({ branches, status }) {
    ...
    const taskCount = branches.reduce((acc, b) => (Array.isArray(b) ? acc + b.length : acc), 0);
                ...
                <div className="px-4 pb-4 space-y-3">
                    {branches.map((item, idx) => {
                        if (typeof item === "string" && item.trim()) {
                            return (
                                <p key={idx} className="text-xs text-gray-400 italic leading-relaxed pl-2 border-l border-blue-500/30">
                                    {item}
                                </p>
                            );
                        }
            ...
            <ul className="space-y-2">
                {decisions.map((decision, idx) => (
                    <li key={idx} className="flex items-start gap-2">
                        <span className="text-purple-400 text-xs mt-0.5 flex-shrink-0">{idx + 1}.</span>
...
export default function MpptChat() {
    ...
    useEffect(() => {
        ...
        if (initialPrompt && messages.length === 0 && !isLoadingHistory) {
            const internetSearch = localStorage.getItem("mpptInternetSearch") !== "false";
            localStorage.removeItem("mpptPrompt");
            localStorage.removeItem("mpptInternetSearch");
            handleNewMessage({ prompt: initialPrompt, isInternetSearch: internetSearch });
        }
    }, [isLoadingHistory]);
    ...
    const startPolling = useCallback((jobId) => {
        ...
        const poll = async () => {
            try {
                ...
                if (res.success && res.data) {
                    const { status, answer, branches, decisions, quantumRankings } = res.data;
                    ...
                    setMessages((prev) => {
                        ...
                        if (idx !== -1) {
                            updated[idx] = {
                                ...
                                quantumRankings: quantumRankings || updated[idx].quantumRankings || [],
                                status,
                            };
                        }
                        return updated;
    ...
    const handleNewMessage = async ({ prompt, isInternetSearch }) => {
        ...
        for (let i = 0; i < messages.length - 1; i += 2) {
            ...
            if (
                ...
                chatMemory.push({ role: "user", content: userMsg.prompt.slice(0, 5000) });
                chatMemory.push({ role: "assistant", content: assistantMsg.answer.slice(0, 5000) });
            }
        }
        ...
        try {
            ...
            toast({ title: "Error", description: error.message || "Failed to start analysis", variant: "destructive" });
            setIsPolling(false);
            setMessages((prev) => {
                const updated = [...prev];
                const last = updated[updated.length - 1];
