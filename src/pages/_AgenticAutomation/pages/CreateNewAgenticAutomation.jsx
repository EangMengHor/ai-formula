...
const a = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        query:
          "Domain categorization (Finance, Science, Technology, Health, etc.)",
      },
      {
...
const polishedApiData = z.object({
  name: z
    .string()
    .min(3, { message: "Automation name must be at least 3 characters." })
    .max(50, { message: "Automation name must be at most 50 characters." }),
  task: z
    .string()
  ...
  interval: z.enum(["daily", "weekly", "monthly"], {
    message: "Please select an execution interval From the given options!",
  }),
  workflow: z.array(
    z.object({
      name: z.string(),
...
export default function CreateAutomationForm() {
  ...
  const { selectedSuperiorPersona, user } = useUser();
  ...
  const applyWorkflowChanges = async () => {
    ...
    try {
      const result = await applyChangesToWorkflow(
        workflowSteps,
        workflowChanges,
      );
  ...
  const deleteChildStep = (parentId, childIndex) => {
    ...
    const updatedSteps = workflowSteps.map((step) => {
      if (step.id === parentId) {
        ...
        const updatedSubSteps = step.steps.filter(
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message ||
          "There was an error creating your automation. Please try again.",
        variant: "destructive",
        duration: 5000,
      });
    } finally {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      ...
      >
        Configure your automation with specific tasks and behaviors. Fill out
        the form below to create a new automation that will run according to
        your specifications.
      </motion.p>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    ...
                    <FormDescription className="text-sm text-gray-400">
                      Describe what this automation should accomplish and any
            ...
            <motion.div variants={itemVariants}>
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-5xl h-[80%] bg-slate-700 p-0 border-2 border-slate-500 overflow-y-scroll">
                    <GroupSuperiorPersonaSection
                      onFetchSuperiorPersona={setOnFetchSuperiorPersona}
                      isShowInteractionOptions={false}
                    />
                  </DialogContent>
                </Dialog>
              </div>
              {selectedSuperiorPersona &&
                selectedSuperiorPersona.length > 0 && (
                  <p className="font-semibold text-md text-white mt-4 mb-2">
                    Selected Superior Persona
                  </p>
                )}
              {selectedSuperiorPersona &&
                selectedSuperiorPersona.length > 0 && (
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-center text-gray-400 text-sm">
                                ...
                                <span>
                                  Created {formatDistanceToNow(item.date)} ago
                                </span>
                              </div>
                            </div>
                          </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2 bg-slate-600 px-2 py-3 rounded-md">
                    ...
                    <FormDescription className="text-sm text-gray-400">
                      if Turned on then workflow will have context of previously
                      executed workflow
                    </FormDescription>
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        <FormLabel className="text-sm font-medium text-white">
                          Number of Previous Workflows
                        </FormLabel>
                        <FormDescription className="text-sm text-gray-400">
                          Select how many previous workflow executions to
                          include as context for new execution (1-5).
                        </FormDescription>
                ...
                render={({ field }) => (
                  <FormItem className="space-y-4">
                    <div className="flex items-center gap-2">
                      <FileText className="h-5 w-5 text-gray-400" />
                      <FormLabel className="text-lg font-medium text-white">
                        Assign Workflow To Each Agent
                      </FormLabel>
                    </div>
                    <FormDescription className="text-sm text-gray-400">
                      Selected Workflow will be assigned to each agent for
                      performing broken down service of your given task
                    </FormDescription>
                    <FormControl>
                      <Tabs
                        value={field.value}
                                ...
                                render={({ field }) => (
                                  <FormItem className="mb-4">
                                    ...
                                    <FormDescription className="text-xs text-gray-500 mt-2">
                                      ...
                                      workflow steps and we'll automatically
                                      process them.
                                    </FormDescription>
                                    <FormMessage className="text-red-400" />
                                  </FormItem>
          ...
          >
            ...
            >
              <span
                className={`${isSubmitting ? "opacity-0" : "opacity-100"} transition-opacity`}
              >
                Create Automation
              </span>
              {isSubmitting && (
            ...
            {!isFormValid() && (
              ...
              >
                ...
                <span>
                  Please complete all required fields and process a workflow
                  before submitting
                </span>
              </motion.div>
            )}
      ...
      <Dialog open={isFinalDialogOpen} onOpenChange={setIsFinalDialogOpen}>
        <DialogContent className="max-w-[60%] bg-purple-700">
          ...
          <Alert className=" bg-purple-800 text-white">
            ...
            <AlertDescription>
              Your Workflow is under process and and ai agent is using knowledge
              base and creating actionable Agent Plan For Whole Workflow
            </AlertDescription>
          </Alert>
          <div>
            <ul className="list-disc text-white ml-6">
              <li>
                The AI Agent reviews your workflow and extracts data from the
                knowledge base.
              </li>
              <li>It then creates an actionable plan based on this data.</li>
              <li>You Can Safely Close And Leave This Page</li>
              <li>
                This process takes time, so it won't appear on your dashboard
                immediately.
              </li>
            </ul>
          </div>
