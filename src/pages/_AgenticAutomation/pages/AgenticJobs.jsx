...
import { CalendarIcon, Copy, ExternalLink, Loader2, RefreshCcw } from "lucide-react"
...
export default function AgenticJobs() {
    ...
    // info section
    const [isAutomationSectionLoading, setIsAutomationSectionLoading] = useState(false)
    const [automation, setAutomation] = useState(null)
    const fetchJobsOfAutomation = useCallback(throttle(async () => {
    ...
    useEffect(() => {
        if (automationId) {
            Promise.all([
                fetchAutomationSection(),
                fetchJobsOfAutomation()
            ])
        }
    ...
    const getStatusBadge = (status) => {
        return (
            <div className="flex items-center gap-2">
                <Badge variant="secondary" className={`bg-blue-500/10 text-blue-500 ${getStatusColor(status)}`}>
                    <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Select value={statusFilter} onValueChange={setStatusFilter}>
                                    <SelectTrigger className="w-[180px] bg-slate-950 border-slate-800 text-slate-300 focus:ring-slate-700 focus:ring-offset-slate-900">
                                        <SelectValue placeholder="Filter by status" className="text-slate-400" />
                                    </SelectTrigger>
                                    <SelectContent className="bg-white border-slate-200">
                                        <SelectItem value="all" className="text-slate-900 focus:bg-slate-100 focus:text-slate-900">
                                            All Statuses
                                        </SelectItem>
                                        <SelectItem value="running" className="text-slate-900 focus:bg-slate-100 focus:text-slate-900">
                                            Running
                                        </SelectItem>
                                        <SelectItem value="completed" className="text-slate-900 focus:bg-slate-100 focus:text-slate-900">
                                            Completed
                                        </SelectItem>
                                    ...
                                    <PopoverContent className="w-auto p-0">
                                        <Calendar
                                            ...
                                            classNames={{
                                                ...
                                                day_selected: "bg-blue-500 text-slate-50 hover:bg-blue-500 hover:text-slate-50 focus:bg-blue-500 focus:text-slate-50",
                                                day_today: "bg-slate-800 text-slate-50",
                                                day_outside: "text-slate-500 opacity-50",
                                                day_disabled: "text-slate-500",
                                                day_range_middle: "aria-selected:bg-slate-800 aria-selected:text-slate-50",
                                                day_hidden: "invisible",
                                                nav_button: "bg-slate-800 text-slate-400 hover:bg-slate-700",
                                                nav_button_previous: "absolute left-1 p-2 rounded-md",
                                                nav_button_next: "absolute right-1 p-2 rounded-md",
                                                caption: "relative text-slate-200 flex justify-center items-center"
                                            }}
                                        />
                                    </PopoverContent>
                                </Popover>
                            ...
                            <Table>
                                <TableHeader className="bg-slate-900">
                                    <TableRow className="border-slate-800 hover:bg-slate-900 ">
                                        <TableHead className="text-slate-400 bg-slate-900">Job ID</TableHead>
                                        <TableHead className="text-slate-400 bg-slate-900">Created At</TableHead>
                                        <TableHead className="text-slate-400 bg-slate-900">Status</TableHead>
                                        <TableHead className="text-right text-slate-400 bg-slate-900">Actions</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {filteredJobs.length > 0 ? (
                                        filteredJobs.map((job) => (
                                            <TableRow key={job.id} className="hover:bg-slate-950 bg-slate-900">
                                                ...
                                                <TableCell className="text-slate-300">{format(parseISO(job.created_at), "PPP p")}</TableCell>
                                                        ...
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button
                                                                    ...
                                                                    className={`bg-slate-900 text-slate-300 hover:bg-slate-800 ${job.status === "running" ? "cursor-not-allowed opacity-50" : ""
                                                                        }`}
                                                                >
                                                            ...
                                                            {job.status === "running" && (
                                                                <TooltipContent>Cannot view details while job is running</TooltipContent>
                                                            )}
                                                        </Tooltip>
                                                    </TooltipProvider>
                                                </TableCell>
