...
export default function AgenticAutomation() {
    ...
    // local states
    const [isUserAgenticAutomationLoading, setIsUserAgenticAutomationLoading] = useState(false)
    const [userAgenticAutomationProject, setUserAgenticAutomationProject] = useState([])
    const { user } = useUser();
    ...
    return (
        <div>
            ...
            >
                ...
                <p className="font-semibold text-xl mt-3 text-white text-center">Create New Agentic Automation</p>
            </Link>

            <hr className="border border-slate-600" />
            {
                isUserAgenticAutomationLoading ? <div className="w-full h-96 gap-2 flex items-center justify-center">
                    <LoaderCircle className="text-white animate-spin" />
                    <p className="font-semibold text-white ">Loading Your Agentic Automation Project</p>
                </div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 my-4">

                    {
                        userAgenticAutomationProject && userAgenticAutomationProject.length > 0 && userAgenticAutomationProject.map((project, index) => {
                            return <UserAgenticAutoCard
                                key={index}
                                name={project.name}
