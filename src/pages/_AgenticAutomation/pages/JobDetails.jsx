...
import { Brain, Calendar, Clock, ChevronDown, Activity, Maximize2, ChevronRight, LoaderCircle } from 'lucide-react'
...
import { formatDate, getStatusColor, groupWorkflowData } from "../../../lib/utils"
...
export default function JobDetails() {
    ...
    const [eachAgentResponse, setEachAgentResponse] = useState([]); const [isShowAllAgentsLoading, setIsShowAllAgentsLoading] = useState(false)
    ...
    return (
        <div className="min-h-screen  text-slate-100 p-6">
                    ...
                    <div className="space-y-4">
                        <div className="bg-slate-900 rounded-xl border border-slate-700/50 p-6 h-[400px]">
                            <div className="text-slate-400 mb-4">
                                <h3 className="font-semibold text-slate-300 text-xl">Agent Interaction Behaviour</h3>
                                {jobData.jobInteractionMap.length || 0} Interaction Taken Place In This Job. Check Goal Achieved By Each Agent By Zooming the Agents Below 👇
                            </div>
                            <div className="h-[calc(100%-30px)]">
                                <WorkflowDiagram isAgenticWorkflowExecuted={true} data={groupWorkflowData(jobData.jobInteractionMap)} />
                            </div>
                        </div>
                        <Button
                            variant="outline"
                    ...
                    <div className="bg-slate-900/50 rounded-xl border border-slate-700/50 p-6">
                        ...
                        <div className="mb-6 bg-slate-800 p-4 rounded-md">
                            <div className="flex items-start justify-between gap-4 mb-3">
                                <h1 className="text-2xl font-semibold text-white truncate" title={jobData.automationName}>
                                    {jobData.automationName}
                                </h1>
                                <Badge className={`px-3 py-1 border ${getStatusColor(jobData.jobStatus)}`}>
                                    {jobData.jobStatus}
                                </Badge>
                            </div>
                            <p className="text-slate-400 line-clamp-2" title={jobData.automationTask}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-white">{formatDate(jobData.jobCreatingDate)}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {jobData.automationSuperiorPersona.map((persona, index) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-slate-300 truncate max-w-[200px]">{persona.name}</span>
                                            <Badge className="bg-slate-700 text-slate-300 border-none">{persona.numberOfPersona}</Badge>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <div className="flex gap-2 w-full mt-6">
                    ...
                    {jobData.jobFinalReport && jobData.jobFinalReport !== "''" && (
                        <div className=" bg-slate-900/50 rounded-xl border border-slate-700/50 p-6 w-[60%]">
                            ...
                            <div className="relative">
                                <div
                                    className={`prose prose-invert max-w-none bg-slate-900/30 rounded-lg py-4 ${!showFullReport ? "max-h-[180px] overflow-hidden" : ""
                                        }`}
                                >
                                    <ReactMarkdown
                                ...
                                <div className="mt-8 text-center">
                                    ...
                                    >
                                        ...
                                        <ChevronDown className={`ml-2 h-4 w-4 transition-transform ${showFullReport ? "rotate-180" : ""}`} />
                                    </Button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-900 rounded-xl border border-slate-700/50 p-6  w-[40%]">
                        <div>
                            ...
                            <p className="text-slate-400 capitalize">This is Workflow that Each Agent Have Used to reach to decisions , research and final response</p>
                        </div>
            ...
            <div className="my-2">
                <div onClick={getAllAgentsResponseAndCoT} className={`flex justify-between cursor-pointer bg-slate-900/50 rounded-xl border border-slate-700/50 p-6 font-semibold text-lg hover:bg-slate-700 ${eachAgentResponse.length > 0 && 'hidden'}`}>
                    {
                    ...
                    eachAgentResponse.length > 0 && (
                        <div>
                            <div className="flex gap-2 text-white items-center">
                                <p className="font-semibold text-lg">Personas Used In This Automation</p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3  gap-4 mt-4 pb-80">
                                ...
                                {
                                    eachAgentResponse.length > 0 && eachAgentResponse.map((agent, index) => (
                                        <PersonaOp key={index} output={agent.output} title={agent.name} goal={agent.goal} team={agent.team} cot={agent.CoT} />
                                    ))
                                }
            ...
            <Dialog open={showFullDiagram} onOpenChange={setShowFullDiagram}>
                <DialogContent className="max-w-[90vw] max-h-[90vh] bg-slate-900 p-6">
                    <div className="w-full h-[calc(90vh-100px)]">
                        <WorkflowDiagram isAgenticWorkflowExecuted={true} data={groupWorkflowData(jobData.jobInteractionMap)} isFullSize={true} />
                    </div>
                </DialogContent>
            </Dialog>
        </div>
