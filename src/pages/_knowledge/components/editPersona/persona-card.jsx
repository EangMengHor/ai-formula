      ...
      <Card className="bg-slate-900 border-slate-600">
        <CardHeader className="">
          <div className="flex justify-between gap-2 items-center">
            ...
            <HoverCard open={open} setOpen={setOpen}>
              ...
              {isEditLoading || KnowledgeLoading ? (
                <div className="bg-slate-700 p-4 rounded-md flex gap-2 font-semibold">
                  <LoaderCircle className="animate-spin" />
                  {sources.length > 0 ? (
                    <p className="text-slate-400">Storing Knowledge</p>
                  ) : (
                ...
                >
                  <Button onClick={() => setOpen(!open)}>
                    {open ? "Click To Close" : "Edit With AI"}
                  </Button>
                </HoverCardTrigger>
              )}
              <HoverCardContent className="w-96 h-fit bg-slate-800/20 backdrop-blur-lg border border-gray-700 p-4 rounded-lg shadow-lg justify-between">
                ...
                <div className="flex w-96 flex-wrap gap-2">
                  {editableField.map((field, index) => (
                    <div
                      key={index}
                      className={`${selectedEditableField.includes(field.value) ? "" : ""} flex gap-2 items-center font-medium cursor-pointer border border-slate-500 rounded-md`}
                    >
                      <Toggle
                ...
                <Separator className="my-4 border border-white" />
                <Button onClick={editField} variant="default">
                  Start Editing
                </Button>
              </HoverCardContent>
            </HoverCard>
        ...
        <CardContent className="space-y-6">
          ...
          <Accordion
            type="single"
            collapsible
            className={`w-full ${isKnowledgeCard ? "hidden" : ""}`}
          >
            ...
            <AccordionItem value="knowledge">
              <AccordionTrigger className="text-slate-200 hover:text-slate-50 flex w-full justify-between items-center">
                <div className="flex gap-2 items-center">
                  ...
                  {editedFields.includes("knowledgeBaseSearch") && (
                    <EditedIcon />
                  )}
                </div>
              </AccordionTrigger>
              <AccordionContent>
                <ScrollArea className="h-[300px] pr-4">
                  {persona.knowledgeBaseSearch &&
                    Array.isArray(persona.knowledgeBaseSearch) &&
                    persona.knowledgeBaseSearch.length > 4 && (
                      <div className="text-sm text-slate-400 px-2 py-1 bg-slate-600 m-2 rounded-md">
                        Scroll For More
                      </div>
                    )}
            ...
            <AccordionItem value="outputFormat">
              ...
              <AccordionContent>
                <div className="text-sm text-slate-400">
                  {persona.outputFormat && persona.outputFormat.trim() === ""
            ...
            <AccordionItem value="sourceLinks">
              ...
              <AccordionContent>
                <div className="text-sm text-slate-400 break-words">
                  {persona.sourceLinks && persona.sourceLinks.trim() === ""
                    ? "No External Source Links Provided"
                    : persona.sourceLinks}
                </div>
              </AccordionContent>
            </AccordionItem>
            <AccordionItem value="externalAccess">
              ...
              <AccordionContent>
                <Badge
                  variant={
                    persona.isExternalAllowed ? "success" : "destructive"
                  }
                >
          ...
          <div>
            <Dialog>
              <DialogTrigger>
                {sources.length > 0 && (
                  <Button>Show {sources.length} Sources</Button>
                )}
              </DialogTrigger>
              <DialogContent className="w-[90%] max-w-4xl bg-slate-800 h-[60%]">
                <DialogHeader>
                  <DialogTitle className="text-2xl text-white">
                    Creating Knowledge Base Using {sources.length} Sources
                  </DialogTitle>
                  <div className="flex gap-2 text-slate-200">
                    <p>Scroll For More Links</p>•<p>Hover to check full link</p>
                    •<p>Click To Visit</p>
                  </div>
