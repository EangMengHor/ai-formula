      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {personas.reverse().map((persona) => (
            <Card
              key={persona.id}
              className="bg-gray-800 hover:bg-gray-700 transition-colors duration-200 shadow-md flex flex-col justify-between items-center"
            >
              <CardContent className="p-5">
                <h3 className="text-lg font-semibold text-gray-100 mb-2">
                  {persona.sup_per_name}
                </h3>
                ...
                <p className="text-sm text-gray-400 line-clamp-2">
                  {persona.sup_per_description}
                </p>
                <div className="flex items-center mt-2 text-gray-300 text-xs">
                  <Clock className="mr-2 h-3 w-3" />
                  <span>
                    Created {formatDistanceToNow(new Date(persona.created_at))}{" "}
                    ago
                  </span>
                </div>
